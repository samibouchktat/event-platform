...
public interface ProviderPackRepository extends JpaRepository<ProviderPack, Long> {
    ...
    List<ProviderPack> findByProviderProfileOrderByCreatedAtDesc(ProviderProfile providerProfile);

    Optional<ProviderPack> findByIdAndProviderProfile(Long id, ProviderProfile providerProfile);

    boolean existsByNameAndProviderProfile(String name, ProviderProfile providerProfile);

    boolean existsByIdAndProviderProfile(Long id, ProviderProfile providerProfile);

    List<ProviderPack> findByProviderProfileAndActiveTrueOrderByCreatedAtDesc(ProviderProfile providerProfile);
    @Query("""
        SELECT p
        FROM ProviderPack p
